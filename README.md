...
|[kakao-ko-gpt_text_generation_test](https://github.com/kobongsoo/GPT-2/blob/master/kakao-ko-gpt_text_generation_test.ipynb)|**kakao-ko-gpt-float16 모델**(gpt-3모델)을 이용한 텍스트 생성 하는 예제|
|[kakao-ko-gpt_text_generation_finetuning](https://github.com/kobongsoo/GPT-2/blob/master/kakao-ko-gpt_summarizer_finetuning.ipynb)|**kakao-ko-gpt-float16 모델**(gpt-3모델) 을 이용한 한국어 text generaion Fine-Tuning 훈련 예제<br>단 GPU 메모리가 부족해 실제 테스트는 못해봄(32batch_size: 32 일때, 32G 정도 필요할것 같음.(예상))|
...
model.deparallelize()
```
-HuggingFace 공식 도큐먼트에서 parallelize 메소드를 제공하는 모델이면, 위와 같은 병렬처리 적용 가능함.
<br> 예: T5, GPT-2등
